	...
	case WM_COMMAND:
		{
			...
			case ID_PORT_COM2:
				...
				break;
			case ID_PORT_COM3:
				myArduino.ChoosePort(hWnd, L"COM9");
				break;
			case ID_SENSOR_NONE:
			...
			case ID_SENSOR_10:
				...
				break;
			case ID_SENSOR_11:
				myCompList.ConnectSensorToComponent(myCompList.GetSelectedComponent(),11);
				break;
			//Analog Animation Commands
			case ID_ANALOG_1: 
				myCompList.ConnectSensorToComponent(myCompList.GetSelectedComponent(),12);
				myCompList.GetSelectedComponent()->analogPtr = &myArduino.analogdata;
				break;
			case ID_ANALOG_2:
				myCompList.ConnectSensorToComponent(myCompList.GetSelectedComponent(),13);
				myCompList.GetSelectedComponent()->analogPtr = &myArduino.analogdata1;
				break;
			case ID_ANALOG_3:
				myCompList.ConnectSensorToComponent(myCompList.GetSelectedComponent(),14);
				myCompList.GetSelectedComponent()->analogPtr = &myArduino.analogdata2;
				break;
			case ID_ANALOG_4:
				myCompList.ConnectSensorToComponent(myCompList.GetSelectedComponent(),15);
				myCompList.GetSelectedComponent()->analogPtr = &myArduino.analogdata3;
				break;
			case ID_ANALOG_5:
				myCompList.ConnectSensorToComponent(myCompList.GetSelectedComponent(),16);
				myCompList.GetSelectedComponent()->analogPtr = &myArduino.analogdata4;
				break;
			...
			case ID_PLACE_CCTVCAMERA:
				myCompList.AddComp(new CCTVCamera(1,10,7,orientation2::left,12,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_CORNERPIPE:
				myCompList.AddComp(new CornerPipe(2,17,6,orientation2::top,1,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_CROSSJUNCTION:
				myCompList.AddComp(new CrossJunction(3,22,7,orientation2::left,3,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_HYDRAULICVALVE:
				myCompList.AddComp(new HydraulicValve(4,29,8,orientation2::left,1,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_IRONPIPE:
				myCompList.AddComp(new IronPipe(5,35,7,orientation2::left,7,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_PNEUMATICPUMP:
				myCompList.AddComp(new PneumaticPump(6,45,7,orientation2::left,1,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_SANITIZATIONDRAIN:
				myCompList.AddComp(new SanitizationDrain(7,57,7,orientation2::left,1,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_STORAGETANK:
				myCompList.AddComp(new StorageTank(8,68,6,orientation2::left,0,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_SUSPENDEDCONCRETEGROUNDFLOOR:
				myCompList.AddComp(new SuspendedConcreteGroundFloor(9,5,65,orientation2::left,0,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_TJUNCTION:
				myCompList.AddComp(new T_Junction(10,90,7,orientation2::left,3,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_THREEWAYJUNCTION:
				myCompList.AddComp(new ThreeWayJunction(11,95,7,orientation2::left,1,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_TWOWAYJUNCTION:
				myCompList.AddComp(new TwoWayJunction(12,100,7,orientation2::left,1,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_WATERSOURCE:
				myCompList.AddComp(new WaterSource(13,105,7,orientation2::left,0,0,0), hWnd);
				InvalidateRect(hWnd,&wArea,true);
				break;
			case ID_PLACE_PRESSUREGAUGE:
				myCompList.AddComp(new PressureGauge(14,128,7,orientation2::left,7,0,0), hWnd); //New Component Pressure Guage
				InvalidateRect(hWnd,&wArea,true);
				break;
	...
	case WM_CONTEXTMENU:
		{
			...
			HMENU hMenu1 = CreatePopupMenu(), hAttachMenu1;				// context menu handle
			if(hMenu != INVALID_HANDLE_VALUE)
			{
				...
				if(onComponent >= 0) // mouse was over item
				{
					...
					//Create sub menu for Digital Input
					...
					//Create sub menu for Analog Input															
					hAttachMenu1=CreatePopupMenu();
					AppendMenu(hAttachMenu1, MF_STRING, ID_SENSOR_NONE, TEXT("&Disconnect"));
					AppendMenu(hAttachMenu1, MF_STRING, ID_ANALOG_1, TEXT("&Input 1"));
					AppendMenu(hAttachMenu1, MF_STRING, ID_ANALOG_2, TEXT("&Input 2"));
					AppendMenu(hAttachMenu1, MF_STRING, ID_ANALOG_3, TEXT("&Input 3"));
					AppendMenu(hAttachMenu1, MF_STRING, ID_ANALOG_4, TEXT("&Input 4"));
					AppendMenu(hAttachMenu1, MF_STRING, ID_ANALOG_5, TEXT("&Input 5"));	

					//Attach the popup to the menu
					AppendMenu(hMenu, MF_STRING|MF_POPUP, (UINT) hAttachMenu, TEXT("&Digital Sensor"));
					AppendMenu(hMenu, MF_STRING|MF_POPUP, (UINT) hAttachMenu1, TEXT("&Analog Sensor"));
					// Removal of Unnecessary Rotation
					if(myCompList.GetSelectedComponent()->getId() == 2 || myCompList.GetSelectedComponent()->getId() == 5 ||
					   myCompList.GetSelectedComponent()->getId() == 14){
				...
				else // mouse wasnt over item
				{
					...
					hAttachMenu=CreatePopupMenu();
					hAttachMenu1=CreatePopupMenu();
					AppendMenu(hMenu, MF_GRAYED|MF_POPUP, (UINT) hAttachMenu, TEXT("&Digital Sensor"));
					AppendMenu(hMenu, MF_GRAYED|MF_POPUP, (UINT) hAttachMenu1, TEXT("&Analog Sensor"));
