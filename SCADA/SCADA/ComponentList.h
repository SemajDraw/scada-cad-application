...
#include "PressureGauge.h" //New component included
...
void List::Add(char* iComp)
{
	...
	else if(vals[0]==14)//where 14 is a PressureGuage ID
		N->component=new PressureGauge(vals[0],vals[1],vals[2],vals[3],vals[4],vals[5],vals[6]); // New component pressure guage added to component list 
	else
	{
		delete N;
		return;
...
void List::AnimateComponents(HDC hdc, WORD toAnimate)
{
	...
	{
		int inputSel=0;
		while(inputSel<16) //inputs increased to accomidate analog inputs
		{
			WORD mask=0x1;
			mask<<=inputSel;
			if(mask & toAnimate) // toAnimate contains status of digital inputs 12->2 $07fd eg 0000 0111 1111 1101
								 // all not connected (pulled high automatically on board, input 3 connected to gnd)
								 // Animation runs if inputs are not connected i.e. high. 
								 // Animation does not run if inputs are grounded.
			{
				...
				while(tNode)
				{
					if(tNode->component->getFuture2() == inputSel+1) // Future2 contains GUI component linked sensor number 1->11 if linked.
																	 // 0 if not linked.
						tNode->component->animate(hdc, GRID_SIZE);   // Animate if GUI component is linked to a sensor.
					tNode=tNode->Next;
				}
			}
			inputSel++;
